...
class _ViewOrderScreenState extends ConsumerState<ViewOrderScreen> {
  ...
  Widget build(BuildContext context) {
    final viewOrderState = ref.watch(viewOrderProvider);
    final viewOrderNotifier = ref.watch(viewOrderProvider.notifier);
    final formData = ref.watch(formDataStoreProvider);

    return viewOrderState.when(
      loading: () => Scaffold(
        ...
        if (error.toString() == ResponseMessage.NO_INTERNET_CONNECTION) {
          return Scaffold(
            body: NoInternetWidget(
              onRetry: () {
                if (widget.orderId != "0" || widget.orderId != "") {
                  viewOrderNotifier.getOrderById("${widget.orderId}");
                }
              },
            ),
          );
